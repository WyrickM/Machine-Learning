...
def read_data():
    ...
    with open(
        "/content/gdrive/My Drive/WSU/CPTS/CPTS 437/Colab Notebooks/HW3/movies.csv"
    ) as csv_file:
    ...
    # read movie plots
    with open(
        "/content/gdrive/My Drive/WSU/CPTS/CPTS 437/Colab Notebooks/HW3/plots-imdb.csv"
    ) as csv_file:
        csv_reader = csv.reader(csv_file)
        line_num = 0
        for row in csv_reader:
    ...
    with open(
        "/content/gdrive/My Drive/WSU/CPTS/CPTS 437/Colab Notebooks/HW3/ratings-imdb.csv"
    ) as csv_file:
        csv_reader = csv.reader(csv_file)
        line_num = 0
        for row in csv_reader:
    ...
    with open(
        "/content/gdrive/My Drive/WSU/CPTS/CPTS 437/Colab Notebooks/HW3/ratings.csv"
    ) as csv_file:
        csv_reader = csv.reader(csv_file)
        line_num = 0
        for row in csv_reader:
...
def movie_feature_vector(movieId):
    ...
    return (
        movie_year_fv
        + genre_fv
        + list(title_fv.toarray()[0])
        + list(plot_fv.toarray()[0])
    )
...
def extract_text_features(dataset):
    ...
    titles_count_vect.set_params(
        max_df=0.5
    )  # ignore terms that appear in >50% of the titles
    ...
    plots_count_vect.set_params(
        max_df=0.5
    )  # ignore terms that appear in >50% of the plots
...
def build_training_set(dataset):
    ...
    for example in dataset:
        ...
        w.append(
            abs(rating_diff)
        )  # cost of misclassification proportional to difference in ratings
    X_data_arr = numpy.array(X_data)
    X_row_arr = numpy.array(X_row)
    X_col_arr = numpy.array(X_col)
    X = scipy.sparse.csr_matrix(
        (X_data_arr, (X_row_arr, X_col_arr)), shape=(num_egs, len(fvec))
    )
...
def get_imdb_ranking(ranking):
    ...
    imdb_ratings = list(
        map(lambda pair: (pair[0], movie_imdb_rating[pair[0]]), ranking)
    )
...
def print_ranking(ranking):
    ...
    for movie_score_pair in ranking:
        ...
        print(
            str(rank) + ". (" + str(score) + ") " + title + " (id=" + str(movieId) + ")"
        )
        rank += 1


def mean_reciprocal(ranking1, ranking2):
    ...
    for movieId in movieIds1:
        ...
        if movieId in movieIds2:
            rank2 = movieIds2.index(movieId)
            distance = abs(rank1 - rank2) + 1
            mrr += float(1) / float(distance)
        else:
            mrr += 0

        rank1 += 1
    return float(mrr) / float(len(ranking1))


def kemeny(ranking1, ranking2):
    # Kemeny Distance measure.
    global movie_imdb_rating
    movieIds1 = list(map(lambda pair: pair[0], ranking1))
    movieIds2 = list(map(lambda pair: pair[0], ranking2))
    distance = 0
    ...
    if len(movieIds) < 2:
        ...
        for movieId in movieIds:
            if movieId != pivot:
                ...
                y_prob = classifier.predict_proba([fv])[0][
                    class_1_index
                ]  # probability movie preferred to pivot
...
def main():
    ...
    dist = kemeny(naive_ranking, imdb_ranking)
    print(
        "\nNaive Ranking (Kemeny distance to IMDb ranking = " + str(dist) + "):",
        flush=True,
    )
    mrr = mean_reciprocal(naive_ranking, imdb_ranking)
    print(
        "Naive Ranking (Mean reciprocal length to IMDb ranking = " + str(mrr) + "):",
        flush=True,
    )
    ...
    dist = kemeny(ranking2, imdb_ranking2)
    print("\nRanking v2 (distance to IMDb ranking = " + str(dist) + "):", flush=True)
    mrr = mean_reciprocal(ranking2, imdb_ranking2)
    print(
        "Ranking v2 (Mean reciprocal length to IMDb ranking = " + str(mrr) + "):",
        flush=True,
    )
